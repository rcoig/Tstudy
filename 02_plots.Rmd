...
```
Figure 2, Mean Digoxin plasma concentration (linear) over time with flipped axes
```{r}
plot_large <- ggplot(summary_dig_plasma, aes(x = time_hr, y = mean_conc, 
  ...
  geom_errorbar(data = summary_dig_plasma %>% filter(drug_visit_number_tru != 3), 
                aes(ymin = mean_conc, ymax = mean_conc + SD_conc,
                    x = time_hr + as.numeric(factor(drug_visit_number_tru)) * 0.1 - 0.3),
                width = 0, linewidth = 0.2, linetype = "solid") +
  geom_errorbar(data = summary_dig_plasma %>% filter(drug_visit_number_tru == 3), 
                aes(ymin = mean_conc - SD_conc, ymax = mean_conc,
                    x = time_hr + 3 * 0.1 - 0.3),
                width = 0, linewidth = 0.2, linetype = "solid") +
  ...
  scale_y_continuous(breaks = seq(0, 2, by = 0.5), limits = c(-0.2, 2), expand = c(0, 0)) +
...
```
Figure 2, Mean Digoxin plasma concentration (linear) over time without flipped axes
```{r}
plot_large <- ggplot(summary_dig_plasma, aes(x = time_hr, y = mean_conc, 
              group = factor(drug_visit_number_tru), 
              linetype = factor(drug_visit_number_tru))) + 
