...
```
Figure 2, Mean Digoxin plasma concentration (linear) over time.
```{r}
plot_large <- ggplot(dig_summary, aes(x = time_hr, y = mean_conc, 
              group = factor(drug_visit_number_tru), 
              color = factor(drug_visit_number_tru), 
              shape = factor(drug_visit_number_tru), 
              linetype = factor(drug_visit_number_tru))) + 
  geom_point(aes(x = time_hr + as.numeric(factor(drug_visit_number_tru)) * 0.1 - 0.1)) +
  geom_line() + 
  geom_errorbar(aes(ymin = mean_conc, ymax = mean_conc + SD_conc,
                    x = time_hr + as.numeric(factor(drug_visit_number_tru)) * 0.1 - 0.1),
                width = 0, linetype = "solid") +
  geom_segment(aes(x = (time_hr + as.numeric(factor(drug_visit_number_tru)) * 0.1 - 0.1) - 0.1, 
                   xend = (time_hr + as.numeric(factor(drug_visit_number_tru)) * 0.1 - 0.1) + 0.1, 
                   y = mean_conc + SD_conc, yend = mean_conc + SD_conc, 
                   color = factor(drug_visit_number_tru)), linetype = "solid", size = 0.5) +  # Horizontal cap on top
  labs(x = "Time after dose (hours)", 
       y = "Concentration (ng/ml)", 
       title = "",
       color = NULL, linetype = NULL, shape = NULL) +  # Remove legend title
  scale_x_continuous(breaks = c(seq(0, 8, by = 2), 12, 24, 48, 50), limits = c(-0.5, 50), expand = c(0, 0)) +
  scale_y_continuous(breaks = seq(0, 2, by = 0.5), limits = c(-0.1, 2), expand = c(0, 0)) +
  scale_color_manual(values = c("1" = "black", "2" = "gray30", "3" = "gray30"),
                     labels = c("1" = "Pre-testosterone", "2" = "1 month post-testosterone", 
                                  "3" = "3 months post-testosterone")) +
  scale_linetype_manual(values = c("1" = "solid", "2" = "dotted", "3" = "dashed"),
                        labels = c("1" = "Pre-testosterone", "2" = "1 month post-testosterone", 
                                  "3" = "3 months post-testosterone")) +
  scale_shape_manual(values = c("1" = 16, "2" = 2, "3" = 0),
                     labels = c("1" = "Pre-testosterone", "2" = "1 month post-testosterone", 
                                  "3" = "3 months post-testosterone")) +
  theme_classic()+
...
plot_inset <- ggplot(data = dig_summary%>%filter(time_hr<8), aes(x = time_hr, y = mean_conc, group = factor(drug_visit_number_tru))) + 
  ...
  labs(x = "Time after dose (hours)", y = "Concentration ( ng/ml )") + 
  scale_color_manual(values = c("1" = "black", "2" = "gray30", "3" = "gray30")) +
  scale_shape_manual(values = c("1" = 16, "2" = 2, "3" = 0)) +
  theme_clean()+
  theme(plot.title = element_text(hjust = 2), 
              ...
              plot.margin = margin(5, 10, 5, 5)) + 
  scale_x_continuous(breaks = c(seq(0, 6, by = 2)), limits = c(0, 6), expand = c(0, 0))
...
ggsave(filename = "Figures/Figure_2.png", plot = Fig2, width = 12, height = 6, dpi = 600)
