...
data<- read.csv("_data/data_T_redcap_20250108.csv", header = TRUE)

data<- read.csv("_data/data_T_redcap_20250108.csv", header = TRUE)%>% #redcap data
  mutate(date_dv = mdy(date_dv),
         return_date = mdy(return_date)) # Converts the date to a standard format

data_dig <- read.csv("_data/data_T_dig_20240706.csv", header = TRUE)%>% #experimental digoxin data
  select(-X)%>%
  rename(study_id=PID_clean, time_min=Time_clean, drug_visit_number_tru = visits)%>%
...
drawtimes_return <- data %>%
  select(study_id, drug_visit_number_return, return_visit, return_date, return_blood_time)%>%
  ...
  group_by(study_id, drug_visit_number_return) %>%
  summarise(
...
drawtimes_initial <- data %>%
  ...
  select(study_id, drug_visit_number_tru, datetime_dose_dv, datetime_15min_dv, datetime_30min_dv,
  ...
  filter(study_id!="12")#participant 12 is empty data as they dropped the study 
drawtimes_initial
```
...
```{r}
drawtimes <- full_join(drawtimes_initial, drawtimes_return, by = c("study_id", "drug_visit_number_tru"))%>%
    mutate(across(
      .cols = -c(study_id, drug_visit_number_tru, datetime_dose_dv), # Exclude the first three columns
...
drawtimes_long <- drawtimes %>% # Reshape to long format
  ...
  select(study_id,drug_visit_number_tru,time_min,time_min_actual)%>%
...
data_dig_adjusted<-full_join(data_dig,drawtimes_long,by=c("study_id","drug_visit_number_tru","time_min"))%>%
    filter(!(is.na(conc_1ml_adjust)))%>% #remove NAs
  mutate(
    time_min_actual = if_else(time_min == 0, time_min, time_min_actual),
    time_hr_actual = if_else(time_hr == 0.00, time_hr, time_hr_actual),#ensure initial time zero is preserved across the df
    time_min_diff = time_min_actual - time_min,
    time_hr_diff = time_hr_actual - time_hr)%>%
    select(study_id, drug_visit_number_tru, dose, time_min, time_hr, time_min_actual, time_hr_actual,
           time_min_diff, time_hr_diff, conc_1ml_adjust)
```
Save data frame in R format for reloading in other scripts.
```{r}
save(data_dig_adjusted, file = "_output/data_dig_adjusted.RData")
```