...
```
Digoxin renal clearance is estimated by CLRenal = Ae (ng) / AUC0–48h (ng*hr)/ml = (ml/hr)
```{r}
data_dig_urine$Cl_R<-round((data_dig_urine$Ae_48h/data_dig_urine$AUClast),0) #ml/hr
```
Adjusting units from hours to mins. Crcl is ml/min and AUC is ng hr/ml; needs to be ng min/ml
```{r}
data_dig_urine$Cl_R<-data_dig_urine$Cl_R/60 #ml/hr -> ml/min
```
...
data_dig_urine$Cl_S<-round((data_dig_urine$Cl_R - data_dig_urine$fuCrCl),1) #ml/min
```
