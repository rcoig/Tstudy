...
Notebook Description: Adjusting Digoxin data using ncappc and calculating resulting summary data.
...
Lambda_zed
```
Use the lambda_z value to calculate adjusted concentrations based on the actual time points using the equation: C = C(0) ∗ e^(−λ∗τ) where C(0) is the original concentratio.
```{r}
# Update concentration values using the first-order kinetics equation only for 24 and 48 hour values
data_dig_adjusted$conc_1ml_new <- ifelse(data_dig_adjusted$time_hr %in% c(24, 48),
  data_dig_adjusted$conc_1ml_adjust * exp(-data_dig_adjusted$Lambda_z * data_dig_adjusted$time_hr_actual),
  data_dig_adjusted$conc_1ml_adjust
)

plot(data_dig_adjusted$conc_1ml_adjust~data_dig_adjusted$conc_1ml_new)# only lowest concentrations are affected as expected
```
Re-run the ncappc package on the adjusted values to calculate new lambda_zed for ideal time points.
```{r}
ncappc(obsFile = data_dig_adjusted,
       str1Nm = "drug_visit_number_tru",
       str1 = c(1,2,3),
...
Lambda_zed_new <- ncaOutput %>% select(ID, drug_visit_number_tru, Lambda_z)%>%
  rename(Lambda_z_new=Lambda_z)
...
dig_summary <- data_dig_adjusted %>%
  ...
  summarise(mean_conc = mean(conc_1ml_new, na.rm = TRUE), 
            SD_conc = sd(conc_1ml_new, na.rm = TRUE))
```
